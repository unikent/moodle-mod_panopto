...
/**
 * Provides the steps to perform one complete backup of the Box Of Broadcasts instance
 */
class backup_panopto_activity_task extends backup_activity_task {
    ...
    static public function encode_content_links($content) {
        ...
        $base = preg_quote($CFG->wwwroot.'/mod/panopto', '#');

        // Link to the list of panopto.
        $pattern = "#(".$base."\/index.php\?id\=)([0-9]+)#";
        $content = preg_replace($pattern, '$@PANOPTOINDEX*$2@$', $content);

        // Link to panopto view by moduleid.
        $pattern = "#(".$base."\/view.php\?id\=)([0-9]+)#";
        $content = preg_replace($pattern, '$@PANOPTOVIEWBYID*$2@$', $content);
